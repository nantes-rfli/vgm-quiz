...
## v1.13: “1問/日” 自動MVP（埋め込みのみ／Apple優先→YouTube、自前配信なし）
### 目的
- 法務リスクを最小化しつつ、無人で **毎日1問** を安定生成・配信する最小実用構成（MVP）を確立する。

### 追加されるもの
- **埋め込みのみ**での再生に統一（Apple優先→YouTube、いずれもダメなら当日スキップ）。
- クリップの開始秒選定（既存ヒューリスティクスを流用）。
- 難易度スコア付与、誤答肢（ディストラクタ）生成、表記正規化（JP/EN/略称・全/半・ローマ数字）。
- 一意性ロック（重複・類似の連発回避）。
- 生成物（Daily JSON / OGP / Feeds）の自動発行。
- 運用復旧手順の明文化（`docs/OPERATIONS_DAILY_ONEQ.md`）。
- KPI の Step Summary 出力（成功率・生成LT・メディア解決率・重複拒否件数・ゲート余裕度 等）。

### 変わること
- 再生方式は**埋め込みオンリー**に固定（自前配信禁止）。
- daily パイプラインの自動化度が上がる（人手前提の手戻りを排除）。

### スコープ外（v1.13ではやらない）
- 新規プロバイダ実装（Spotify/Bandcamp 等）。
- 本格クロール（allowlist 雛形以外）。
- 大規模 UI 変更（挙動不変を維持）。

### DoD（受け入れ基準・要点）
- 毎日自動で1問を生成し、**埋め込みのみ**で再生できる。
- 失敗時に手順書どおり復旧できる（手動再実行／強制skip／ロールバック）。
- Step Summary に KPI が毎回出力される。
- 既存ガード（E2E／Budgets／Docs）が緑。

### リスクと対策
- メディア未解決 → 翌日繰越 or 強制skip の運用ルール化。
- ディストラクタ品質ブレ → 距離ヒューリスティクス＋NGリストで抑制。
- 同一作品の連発 → 一意性ロックに「直近N日回避」を追加。
