...
- **Required checks（3本）**: `pages-pr-build`, `ci-fast-pr-build`, `required-check`（**e2e (light required)** のジョブ）
  - GitHub UI では **`e2e (light required) / required-check`** など *workflow名 / job名* として表示される場合がありますが、Rulesets では **job名 `required-check`** を選べばOK。
  - 新しく追加したチェックは **一度実行しないと Rulesets に出ない** ため、Actions から手動実行してから設定してください。
  - ジョブ名/ワークフロー名を変更した場合は **Rulesets** 側の Required も更新しないと PR が永遠に「waiting」になります。
