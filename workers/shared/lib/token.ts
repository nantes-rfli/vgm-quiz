/**
 * Token utilities for Phase 1 (Base64) and Phase 2B (JWS)
 * Shared library used by both API Worker and MSW mocks
 */

/**
 * Phase 2B Token Payload (JWS signed)
 * Based on docs/api/rounds-token-spec.md
 */
...
export async function verifyJWSToken(
  ...
  try {
    ...
    const isValid = await crypto.subtle.verify(
      'HMAC',
      key,
      signatureBuffer as ArrayBufferView,
